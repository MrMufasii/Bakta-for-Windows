...
sub check_infernal_version
{
    ...
    # Portable: run "<bin> -h" and filter for the INFERNAL banner in Perl
    # (avoids "| grep", which is unavailable under cmd.exe on native Windows).
    my @lines = `"$exec_bin" -h`;
    my $version = 0;
...
sub cmsearch_scoring
{
    ...
    # Portable file slurp (was `cat $cms_output_file`, which fails under cmd.exe)
    @cms_lines = ();
    if (open(my $CMSFH, "<", $cms_output_file))
    {
        @cms_lines = <$CMSFH>;
        close($CMSFH);
        chomp(@cms_lines);
